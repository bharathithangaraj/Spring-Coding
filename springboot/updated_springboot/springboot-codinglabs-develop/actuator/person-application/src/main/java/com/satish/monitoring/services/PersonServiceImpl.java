...
public class PersonServiceImpl implements PersonService {
	...
	public Optional<Person> saveUpdatePerson(Person person) {
		if(person.getPersonId() == 0 || !personRepo.existsById(person.getPersonId())){
			...
			PersonEntity personEntity =new PersonEntity(person.getPersonId(),person.getFirstName(),person.getLastName(),person.getEmail());

			PersonEntity entity = personRepo.save(personEntity);
